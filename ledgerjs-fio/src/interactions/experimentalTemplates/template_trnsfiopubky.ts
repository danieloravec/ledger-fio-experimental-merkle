...
import { baseToCommandMapping } from "./utils/baseToCommandMapping";
...
export function template_trnsfiopubky(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
    ...
    const template_base = template_base_trnsfiopubky();
    const filled_template = template_base.map(commandInfo => {
        const restArgs = commandInfo.getVarArgs(tx);
        const command = baseToCommandMapping(commandInfo.commandBase, restArgs);
        if (!command) {
            throw new Error("Unexpected error: command not found");
        }
        return command;
    });
